...
def determine_if_fit_converged(
    dic_rms, catalog, observation, wcsprm, NAXIS1, NAXIS2, silent=False
):
    ...
    if not silent:
        print(
            "Conditions for converegence of fit: at least 5 matches within {}px and at least 0.2 * minimum ( observed sources, catalog sources in fov) matches with the same radius".format(
                dic_rms["radius_px"]
            )
        )
...
def find_sources(
    image,
    vignette=3,
    vignette_rectangular=1.0,
    cutouts=None,
    sigma_threshold_for_source_detection=5,
    only_rectangle=None,
    FWHM=4,
):
    ...
    if vignette < 3:
        ...
        mask = (x[np.newaxis, :] - sidelength / 2) ** 2 + (
            y[:, np.newaxis] - sidelength / 2
        ) ** 2 < vignette**2
        image[~mask] = median

    # ignore a fraction of the image at the corner
    if vignette_rectangular < 1.0:
        sidelength_x = image.shape[1]
        sidelength_y = image.shape[0]
        cutoff_left = (1.0 - vignette_rectangular) * sidelength_x
        cutoff_right = vignette_rectangular * sidelength_x
        cutoff_bottom = (1.0 - vignette_rectangular) * sidelength_y
    ...
    daofind = DAOStarFinder(
        fwhm=FWHM,
        threshold=sigma_threshold_for_source_detection * std,
        brightest=s.N_BRIGHTEST_SOURCES,
    )
    if s.DETECTION_ABSOLUTE_THRESHOLD is not None:
        daofind = DAOStarFinder(
            fwhm=FWHM,
            threshold=s.DETECTION_ABSOLUTE_THRESHOLD,
            brightest=s.N_BRIGHTEST_SOURCES,
        )
    ...
    xcenters = xcenters + 1.0
    ycenters = ycenters + 1.0
...
def write_wcs_to_hdr(original_filename, wcsprm, report, hdul_idx=0):
    ...
    with fits.open(original_filename) as hdul:
        ...
        hdr_file["AST_CONV"] = (
            report["converged"],
            "T or F for astroemtry converged or not",
        )
...
def read_additional_info_from_header(
    wcsprm,
    hdr,
    RA_input=None,
    DEC_input=None,
    ...
    keywords_check = [
        ...
        "RA",
        "DEC",
    ]  # list of possible keywords the scs parser might miss
    ...
    if (
        (np.abs(wcs_pixscale[0])) < 1e-7
        or (np.abs(wcs_pixscale[1])) < 1e-7
        or (np.abs(wcs_pixscale[0])) > 5e-3
        or (np.abs(wcs_pixscale[1])) > 5e-3
    ):
        if not silent:
    ...
    if "PIXSCALE" in keywords_present:
        ...
        if 20 > x_size > 0.5 and 20 > y_size > 0.5:
            ...
            if (
                wcs_pixscale[0] / pixscale < 0.1
                or wcs_pixscale[0] / pixscale > 10
                or wcs_pixscale[1] / pixscale < 0.1
                or wcs_pixscale[1] / pixscale > 10
            ):
    ...
    if np.array_equal(wcsprm.crval, [0, 0]):
        ...
        if (
            "RA" in keywords_present and "DEC" in keywords_present
        ):  ##carefull degree and hourangle!!!
            ...
            if not silent:
                print(
                    "Is this position within the field of view in degrees? otherwise it will not work. In that case give a more accurate position as an argument: -ra XX -dec XX both in degrees"
                )
    ...
    if np.array_equal(wcsprm.crval, [0, 0]):
        print(">>>>>>>>>WARNING")
        print(
            "No rough sky position was found for this object. Please add as -ra XX -dex XX both in degress. Adding the position as keywords in the fits file header will also work. The keywords are RA and DEC. The program expects the values in degrees. "
        )
    ...
    if np.array_equal(wcsprm.ctype, ["", ""]):
        ...
        if projection_ra is not None and projection_dec is not None:
            ...
            print(">>>>>>>>>WARNING")
            print(
                "The wcs in the header has no projection specified. Will guess 'RA---TAN', 'DEC--TAN' (gnomonic projection) if this is incorrect the fit will fail. You can specify the projection via -projection_ra XX -projection_dec XX"
            )
            print(
                "make sure you do not use quotations, example: -proj1 RA---TAN -proj2 DEC--TAN"
            )
...
def parseArguments():
    ...
    parser.add_argument(
        "input",
        nargs="+",
        help="Input Image with .fits ending. A folder or multiple Images also work",
        type=str,
    )
    parser.add_argument(
        "-c",
        "--catalog",
        help="Catalog to use for position reference ('PS', '2MASS' or 'GAIA') or name of local file",
        type=str,
        default="PS",
    )
    ...
    parser.add_argument(
        "-s",
        "--save_images",
        help="Set True to create _image_before.pdf and _image_after.pdf",
        type=bool,
        default=False,
    )
    parser.add_argument(
        "-p", "--images", help="Set 0 to not create the plots.", type=int, default=1
    )

    parser.add_argument(
        "-v",
        "--verbose",
        help="More console output about what is happening. Helpfull for debugging.",
        type=bool,
        default=False,
    )
    parser.add_argument(
        "-silent",
        "--silent",
        help="Almost no console output. Use when running many files. turns verbose off, turns plots off, turns ignore warning on. ",
        type=bool,
        default=False,
    )
    ...
    parser.add_argument(
        "-w",
        "--ignore_warnings",
        help="Set False to see all Warnings about the header if there is problems. Default is to ignore most warnings.",
        type=bool,
        default=True,
    )
    ...
    parser.add_argument(
        "-ra",
        "--ra",
        help="Set ra by hand in degrees. Should not be necessary if the info is in the fits header.",
        type=float,
        default=None,
    )
    parser.add_argument(
        "-dec",
        "--dec",
        help="Set dec by hand in degrees. Should not be necessary if the info is in the fits header.",
        type=float,
        default=None,
    )
    ...
    parser.add_argument(
        "-proj1",
        "--projection_ra",
        help="Set the projection by hand. Should not be necessary if the info is in the fits header. example: -proj1 RA---TAN -proj2 DEC--TAN",
        type=str,
        default=None,
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "-rot_scale",
        "--rotation_scaling",
        help="By default rotation and scaling is determined. If wcs already contains this info and the fit fails you can try deactivating this part by setting it to 0",
        type=int,
        default=1,
    )
    parser.add_argument(
        "-xy_trafo",
        "--xy_transformation",
        help="By default the x and y offset is determined. If wcs already contains this info and the fit fails you can try deactivating this part by setting it to 0",
        type=int,
        default=1,
    )
    parser.add_argument(
        "-fine",
        "--fine_transformation",
        help="By default a fine transformation is applied in the end. You can try deactivating this part by setting it to 0",
        type=int,
        default=1,
    )
    ...
    parser.add_argument(
        "-vignette",
        "--vignette",
        help="Do not use corner of the image. Only use the data in a circle around the center with certain radius. Default: not used. Set to 1 for circle that touches the sides. Less to cut off more",
        type=float,
        default=3,
    )
    parser.add_argument(
        ...
        help="Do not use corner of the image. Cutoff 1- <value> on each side of the image. Default: not used. 1 uses the full image, 0.9 cuts 10%% on each side",
        type=float,
        default=1.0,
    )
    ...
    parser.add_argument(
        "-cutout",
        "--cutout",
        help="Cutout bad recangle from Image. Specify corners in pixel as -cutout xstart xend ystart yend. You can give multiple cutouts ",
        nargs="+",
        action="append",
        type=int,
    )
    ...
    parser.add_argument(
        "-ignore_header_rot",
        "--ignore_header_rot",
        help="Set to 0 to ignore rotation information contained in header",
        type=int,
        default=0,
    )
    ...
    parser.add_argument(
        "-r",
        "--radius",
        help="Change the download radius for how many catalog objects should be downloaded in arcmin. ",
        type=float,
        default=-1,
    )
    parser.add_argument(
        "-save_bad_result",
        "--save_bad_result",
        help="Save result even when it is considered bad by setting this to 1. By default bad results will not be saved",
        type=int,
        default=0,
    )
    ...
    parser.add_argument(
        ...
        "--sigma_threshold_for_source_detection",
        help="Sigma threshold for source detection on the image",
        type=float,
        default=5,
    )
    parser.add_argument(
        "-seeing",
        "--seeing",
        help="Average FWHM of sources. Default 4px",
        type=float,
        default=4,
    )
    ...
    parser.add_argument(
        "-high_res",
        "--high_resolution",
        help="Set true to indicate higher resolution image like HST to allow larger steps for fine transformation. Default False for smaller telescopes",
        type=bool,
        default=False,
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "-filename_for_sources",
        "--filename_for_sources",
        help="Save the sky positions of the sources to file to calibrate another image with it. Set to the filename without extension. Default: not used",
        type=str,
        default=None,
    )
    ...
    # Print version
    parser.add_argument(
        "--version", action="version", version="%%(prog)s - Version 1.5"
    )  #
    # changelog
...
def main():
    ...
    for fits_image_filename in fits_image_filenames:
        ...
        result, _ = astrometry_script(
            fits_image_filename,
            catalog=args.catalog,
            rotation_scaling=0,
            xy_transformation=args.xy_transformation,
            fine_transformation=args.fine_transformation,
            images=images,
            vignette=args.vignette,
            vignette_rectangular=args.vignette_rectangular,
            cutouts=args.cutout,
            ra=args.ra,
            dec=args.dec,
            projection_ra=args.projection_ra,
            projection_dec=args.projection_dec,
            verbose=verbose,
            save_images=args.save_images,
            ignore_header_rot=args.ignore_header_rot,
            radius=args.radius,
            save_bad_result=args.save_bad_result,
            silent=args.silent,
            sigma_threshold_for_source_detection=args.sigma_threshold_for_source_detection,
            high_res=args.high_resolution,
            hdul_idx=args.hdul_idx,
        ...
        if (not result) and args.rotation_scaling:
            ...
            result, _ = astrometry_script(
                fits_image_filename,
                catalog=args.catalog,
                rotation_scaling=args.rotation_scaling,
                xy_transformation=args.xy_transformation,
                fine_transformation=args.fine_transformation,
                images=images,
                vignette=args.vignette,
    ...
    if multiple:
        print(">> Final report:")
        print(
            "Processed {} files, {} of them did converge. The following files failed:".format(
                len(fits_image_filenames), converged_counter
            )
        )
        print(not_converged)
    print("-- finished --")
...
def astrometry_script(
    filename,
    catalog="PS",
    rotation_scaling=True,
    xy_transformation=True,
    fine_transformation=True,
    images=False,
    vignette=3,
    vignette_rectangular=1.0,
    ...
    ignore_header_rot=False,
    radius=-1.0,
    save_bad_result=False,
    silent=False,
    sigma_threshold_for_source_detection=5,
    high_res=False,
    hdul_idx=0,
    filename_for_sources=None,
    FWHM=4,
):
    ...
    if not silent:
        print(
            ">Info found in the file -- (CRVAl: position of central pixel (CRPIX) on the sky)"
        )
        print(WCS(hdr))
    ...
    wcsprm = WCS(hdr).wcs
    wcsprm_original = WCS(hdr).wcs
    wcsprm, fov_radius, INCREASE_FOV_FLAG, PIXSCALE_UNCLEAR = (
        read_additional_info_from_header(
            wcsprm,
            hdr,
            ra,
            dec,
    ...
    if verbose:
        print(WCS(wcsprm.to_header()))
    coord = SkyCoord(
        wcsprm.crval[0], wcsprm.crval[1], unit=(u.deg, u.deg), frame="icrs"
    )
    if not PIXSCALE_UNCLEAR:
        if (
            wcsprm.crpix[0] < 0
            or wcsprm.crpix[1] < 0
            or wcsprm.crpix[0] > image.shape[0]
            or wcsprm.crpix[1] > image.shape[1]
        ):
            if not silent:
                print("central value outside of the image, moving it to the center")
            coord_radec = wcsprm.p2s([[image.shape[0] / 2, image.shape[1] / 2]], 1)[
                "world"
            ][0]
            coord = SkyCoord(
                coord_radec[0], coord_radec[1], unit=(u.deg, u.deg), frame="icrs"
            )
            # print(wcsprm)
    ...
    if catalog == "GAIA" and catalog_data.shape[0] < 5:
        ...
        if not silent:
            print(
                "Now we have a total of {} sources. Keep in mind that there might be duplicates now  since we combined 2 catalogs".format(
                    catalog_data.shape[0]
                )
            )
    ...
    apertures_catalog = CircularAperture(
        wcsprm.s2p(catalog_data[["ra", "dec"]], 1)["pixcrd"], r=5.0
    )
    # plotting what we have, I keep it in the detector field, world coordinates are more painfull to plot
    if images:
        fig = plt.figure()
    ...
    if rotation_scaling:
        if not silent:
            print("Finding scaling and rotation")
        wcsprm = register.get_scaling_and_rotation(
            observation,
            catalog_data,
            wcsprm,
    ...
    if xy_transformation:
        if not silent:
            print("Finding offset")
        wcsprm, _, _ = register.offset_with_orientation(
            observation,
            catalog_data,
            wcsprm,
            fast=False,
            INCREASE_FOV_FLAG=INCREASE_FOV_FLAG,
            verbose=verbose,
            silent=silent,
        )
    ...
    obs_x, obs_y, cat_x, cat_y, distances = register.find_matches(
        observation, catalog_data, wcsprm, threshold=compare_threshold
    )  # 3
    ...
    if fine_transformation:
        ...
        skip_rot_scale = True
        for i in lis:
            wcsprm_new, score = register.fine_transformation(
                observation,
                catalog_data,
                wcsprm,
                threshold=i,
        ...
        if not fine_transformation_success:
            if not silent:
                print(
                    "Fine transformation did not improve result so will be discarded."
                )
        else:
            if not silent:
                print("Fine transformation applied to improve result")
    ...
    if not silent:
        print(
            "WCS got scaled by {} in x direction and {} in y direction".format(
                scales[0] / scales_original[0], scales[1] / scales_original[1]
            )
        )
    ...
    rotation_angle = (
        matrix_angle(
            wcsprm.get_pc() @ wcsprm.get_cdelt(),
            wcsprm_original.get_pc() @ wcsprm_original.get_cdelt(),
        )
        / 2.0
        / np.pi
        * 360.0
    ...
    if not silent:
        print(
            "x offset: {} px, y offset: {} px ".format(
                wcsprm.crpix[0] - old_central_pixel[0],
    ...
    if images:
        print(
            "Plotting final figue. Forcing origin to be (1,1) to be consistent with ds9 and fits file conventions."
        )
        fig = plt.figure()
        ...
        plt.ylabel("pixel y direction")
        plt.title(
            "Result - red: catalog sources, blue: detected sources in img \n (Note origin = (1,1))"
        )
        print(image.shape)
        plt.imshow(
            image,
    ...
    # updating file
    converged = determine_if_fit_converged(
        dic_rms,
        catalog_data,
        observation,
        wcsprm,
    ...
    if converged or save_bad_result:
        ...
        if filename_for_sources != None:
            wcs = WCS(wcsprm.to_header())
            observation_on_sky = wcs.wcs_pix2world(
                observation[["xcenter", "ycenter"]], 1
            )
            ...
            catalog_from_obs["mag"] = (
                -1.0 * observation["aperture_sum"]
            )  # this is fine since we only use the mag to order the sources!
    ...
    return (
        converged,
        dic_rms,
    )  # dictionary with short info about fit, "matches" gives a number of objects matched within certain radius
