...
def parseArguments():
    ...
    parser.add_argument(
        "source_image",
        help="Source image for the calibration. The program extracts and saves sources from this image. ",
        type=str,
    )
    parser.add_argument(
        "filename_for_sources",
        help="Filename for the sources to save to (without ending)",
        type=str,
    )
    ...
    parser.add_argument(
        "-t",
        "--target_image",
        help="Target image for the calibration (so only roughly the area of the image is used for source extraction)",
        type=str,
        default=None,
    )
    ...
    parser.add_argument(
        "-target_hdul_idx",
        "--target_hdul_idx",
        help="Index of the target image data in the fits file. Default 0",
        type=int,
        default=0,
    )
    parser.add_argument(
        "-source_hdul_idx",
        "--source_hdul_idx",
        help="Index of the source image data in the fits file. Default 0",
        type=int,
        default=0,
    )
    ...
    parser.add_argument(
        "-det_thres",
        "--detection_threshold",
        help="Detection threshold for sources in sigma above mean. Default 5 sigma",
        type=float,
        default=5,
    )
    parser.add_argument(
        "-padding",
        "--padding",
        help="Padding around target image for extraction. Default 50px",
        type=int,
        default=50,
    )
    parser.add_argument(
...
def main():
    ...
    if args.target_image != None:
        ...
        corners_sky = target_wcs.wcs_pix2world(
            [[0.0, 0.0], [target_image.shape[0], target_image.shape[1]]], 1
        )
    ...
    sources_in_target_image = astrometry.find_sources(
        image,
        only_rectangle=only_rectangle,
        cutouts=args.cutout,
        sigma_threshold_for_source_detection=args.detection_threshold,
    )
    print("Found {} sources".format(sources_in_target_image.shape[0]))

    observation_on_sky = wcs.wcs_pix2world(
        sources_in_target_image[["xcenter", "ycenter"]], 1
    )
    ...
    catalog_from_obs["mag"] = (
        -1.0 * sources_in_target_image["aperture_sum"]
