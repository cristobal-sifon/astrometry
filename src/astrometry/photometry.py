...
def find_sources(image, aperture):
    ...
    # print(bkg_sigma)
    # print(std)
    daofind = DAOStarFinder(fwhm=4.0, threshold=5.0 * std, brightest=200)
...
def parseArguments():
    ...
    parser.add_argument(
        ...
        type=str,
    )  # , default="sample_images/pso016p03_Jrot.fits")  #pso016p03_Jrot.fits")
    ...
    parser.add_argument(
        "-b",
        "--band",
        help="Photometric band to use. Options are g,r,i,z,y,J,K,H",
        type=str,
        default="z",
    )
    parser.add_argument(
        "-c",
        "--catalog",
        help="Catalog to use for photometric reference ('PS'), this can be left to  the standard 'auto' to let the program choose automaticaly",
        type=str,
        default="auto",
    )
    ...
    parser.add_argument(
        "-ra",
        "--ra",
        help="Set ra of the object targeted for observation",
        type=float,
        default=None,
    )
    parser.add_argument(
        "-dec",
        "--dec",
        help="Set dec of the object targeted for observation",
        type=float,
        default=None,
    )
    parser.add_argument(
        "-name",
        "--name",
        help="Provide name of object with coordinates given in targets.csv (comma separated, columns NAME, RA, DEC, both in degrees)",
        type=str,
        default=None,
    )
    ...
    parser.add_argument(
        "-a",
        "--aperture",
        help="Aperture in arcsec to use for extraction. Default 2 arcseconds.",
        type=float,
        default="2",
    )
...
def main():
    ...
    for fits_image_filename in fits_image_filenames:
        ...
        wcsprm = Wcsprm(
            hdr.tostring().encode("utf-8")
        )  # everything else gave me errors with python 3
        ...
        ZP = -1 * (
            -2.5 * np.log10(obs_matched["aperture_sum"].values)
        ...
        if ra and dec:
            ...
            coordinats_obs = wcsprm.p2s(observation[["xcenter", "ycenter"]].values, 1)[
                "world"
            ]
            ...
            sig3_limiting_mag = (
                -2.5 * np.log10(3 * std * np.sqrt(ap_area.area)) + ZP_median
            )
            sig5_limiting_mag = (
                -2.5 * np.log10(5 * std * np.sqrt(ap_area.area)) + ZP_median
            )

            #######################
            # estimating the error via random apertures
            # print(std*np.sqrt(aperture_obj.area()))
            ran_x = 2 * aperture + np.random.random(1000) * (
                image.shape[0] - 4 * aperture
            )
            ran_y = 2 * aperture + np.random.random(1000) * (
                image.shape[0] - 4 * aperture
            )
            ...
            phot_table["aperture_sum"].info.format = (
                "%.8g"  # for consistent table output
            )
            ...
            if cand_dups.sum() > 0:
                print(
                    "Position was given. Found {} sources in a 5 arcsec radius. Here is the magnitude:".format(
                        cand_dups.sum()
                    )
                )
                ...
                print(
                    "We get a signal to noise of {} for the fixed aperture".format(
                        noise
                    )
                )
                ...
                text = "found detection in {} band,\n {:.4g} +- {:.2g} {}mag, {:.3g} S/N \n 5 sig limiting mag is {:.4g}".format(
                    args.band, mag, mag_err, mag_sys, noise, sig5_limiting_mag_apertures
                )
                ...
                forced_mag = (
                    -2.5 * np.log10(forced_phot["aperture_sum"].values) + ZP_median
                )
        ...
        if ra and dec:
            ...
            plt.subplot(1, 2, 2)
            plt.title(
                "Photometric calibration with {} sources from {}".format(
